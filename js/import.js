...
function batchSave(list,fmt){
  ...
  function next(idx){
    if(idx>=batches.length){
      fill.style.width='100%';txt.textContent='Complete! '+done+' studies saved.';
      document.getElementById('dblast').textContent=new Date().toLocaleString();
      toast('✓ '+done+' studies imported');
      setTimeout(function(){wrap.style.display='none';loadData();},2000);return;
    }
    var batch=db.batch();
    batches[idx].forEach(function(s){
      var id=buildDocId(s.pid,s.date,fmt||'MM/DD/YYYY');
      batch.set(db.collection('echo_studies').doc(id),s,{merge:true});done++;
    });
    batch.commit().then(function(){
      var p=Math.round((idx+1)/batches.length*100);
      fill.style.width=p+'%';txt.textContent='Saving '+Math.min(done,list.length)+' / '+list.length+'...';
      next(idx+1);
  ...
  db.collection('echo_studies').get().then(function(snap){
    ...
    allDocs.forEach(function(doc){
      ...
      if(p.length===3){
        ...
        var m,d;if(p0>12){d=p0;m=p1;}else{m=p0;d=p1;}
    ...
    function delNext(idx){
      if(idx>=batches.length){
        ...
        txt.textContent='Done! Removed '+deleted+' duplicates. Database now has '+(allDocs.length-deleted)+' studies.';
        toast('✓ Removed '+deleted+' duplicates');
        setTimeout(function(){wrap.style.display='none';loadData();},3000);return;
      }
      var batch=db.batch();
      batches[idx].forEach(function(id){batch.delete(db.collection('echo_studies').doc(id));deleted++;});
      batch.commit().then(function(){
        var p=50+Math.round((idx+1)/batches.length*50);
        fill.style.width=p+'%';txt.textContent='Deleting... '+deleted+' / '+toDelete.length;
        delNext(idx+1);
      }).catch(function(e){txt.textContent='Error: '+e.message;console.error(e);});
    }
    delNext(0);
