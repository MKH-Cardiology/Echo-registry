// ═══════════════════════════════════════════════════════════
// OCR — ViewPoint Report Scanner (Tesseract.js)
// Improved for screen photos + specific ViewPoint field labels
// ═══════════════════════════════════════════════════════════
...
function parseOCRText(text) {
  ...
  // ── Pre-process: normalise common OCR screen-photo confusions ───────────
  function preProcess(t) {
    return t
      .replace(/E[`''′ʼ]/g, "E'")        // smart quotes on E' labels
      .replace(/e[`''′ʼ]/g, "e'")
      .replace(/E\s*\/\s*E'/g, "E/E'")    // spaces around slash
      .replace(/E\s*\/\s*e'/g, "E/e'")
      .replace(/E\s*\/\s*A/g,  "E/A")
      .replace(/\bl\b/g, '1')             // lone lowercase L → digit 1 in numeric contexts
      .replace(/[\u00AD\u200B\u200C\u200D\uFEFF]/g, '')
      .replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  }
  text = preProcess(text);

  // Helper: first match from array of patterns
  ...
  for (var ci = 0; ci < efCandidates.length; ci++) {
    var cand = efCandidates[ci];
    var em = text.match(cand.re);
    if (em) {
      lvef = (cand.range && em[2]) ? String(Math.round((parseInt(em[1])+parseInt(em[2]))/2)) : em[1];
      efSource = cand.src;
      if (cand.flag) flags.push(cand.flag);
      break;
    }
  }
  ...
  // ── TAPSE — ViewPoint in cm; field expects mm ─────────────────────────────
  ...
  // ── LAVI — ViewPoint labels: LAESVInd MOD BP  |  LAESV Index (A-L)  |  LAESV Index A-L ──
  var lavi = extractAny([
  ...
  // ── Average E/e' — ViewPoint labels: "MV E/E' Avg", "E/E' Avg", "E/e' (avg)", "Avg E/e'" ──
  var ee = extractAny([
